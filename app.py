            ...
            <form id="wizard-form" method="POST" action="{{ url_for('inscricao_unica') }}" autocomplete="off" novalidate>
                    ...
                    <div class="hero-grid"><div class="hero-card">
                        ...
                        <div class="hero-highlights">
                            ...
                            <div class="hero-highlight">
                                ...
                                <div class="benefits-slider" data-benefits-slider>
                                    <div class="benefits-viewport">
                                        <div class="benefit-slide ativo">Cursos 100% gratuitos</div>
                                        <div class="benefit-slide">&#127891; Certificado de conclus&#227;o</div>
                                        <div class="benefit-slide">Professores qualificados e aulas presenciais</div>
                                        <div class="benefit-slide">Aprendizado pr&#225;tico voltado para o mercado de trabalho</div>
                            ...
                            <div class="hero-highlight">
                                ...
                                Programa de qualifica&#231;&#227;o profissional gratuita para ampliar suas oportunidades e fortalecer sua entrada no mercado de trabalho.
                            </div>
                        </div>
                ...
                <section class="wizard-panel" data-step="escolher">
                    <div class="step-card"><h2 class="panel-title">Escolha seu curso</h2>
                        <div class="step-grid step-grid--stacked">
                            <div class="form-group full"><label for="local_id_select">Local *</label><select id="local_id_select"><option value="">Selecione um local</option>{% for local in local_options %}<option value="{{ local.id }}" {% if form_data.get('local_id')==local.id %}selected{% endif %}>{{ local.nome }}</option>{% endfor %}</select><div class="balao-erro" id="local_id-error" hidden></div></div>
                            <div class="form-group full" id="curso-group" style="display:none;"><label for="curso_id">Curso *</label><select id="curso_id" name="curso_id"><option value="">Selecione um curso</option></select><div class="balao-erro" id="curso_id-error" {% if not errors.get('curso_id') %}hidden{% endif %}>{{ errors.get('curso_id','') }}</div></div>
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Wise:wght@400;700;900&display=swap" rel="stylesheet">
    <style>
        :root{--cor-principal:#8b0000;--cor-clara:#fbe8e8;--cor-texto:#5a0000;--sombra-card:0 18px 55px rgba(139,0,0,0.18);}
        body{min-height:100vh;margin:0;background:radial-gradient(circle at top left,rgba(139,0,0,0.15),transparent 32%),linear-gradient(140deg,#fff5f5 0%,#fff 55%,#fbe0e0 100%);font-family:'Wise',Arial,sans-serif;}
        .main-header{border-bottom:4px solid #8b0000;background:rgba(255,255,255,0.92);padding:10px 20px;}
        .header-logos{display:flex;align-items:center;justify-content:center;gap:24px;flex-wrap:wrap;}
        .header-divider{width:1px;height:52px;background:linear-gradient(to bottom,transparent,#e2a3a3,transparent);flex-shrink:0;}
        .logo-prefeitura-topo{height:52px;width:auto;object-fit:contain;}
        .logo-projeto-topo{height:52px;width:auto;object-fit:contain;}
        .confirm-page{width:min(680px,calc(100% - 16px));margin:0 auto;padding:10px 0 20px;text-align:center;}
        .wizard-progress{margin:12px auto 16px;padding:14px;border-radius:28px;background:rgba(255,255,255,0.9);box-shadow:0 12px 30px rgba(139,0,0,0.12);}
        .wizard-track{width:100%;height:14px;border-radius:999px;background:#f5d0d0;overflow:hidden;}
        .wizard-fill{width:100%;height:100%;background:linear-gradient(90deg,#8b0000 0%,#c23b3b 100%);border-radius:999px;}
        .wizard-labels{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:12px;}
        .wizard-label{padding:10px 8px;border:1px solid #e2a3a3;border-radius:16px;background:#fff;color:#8b0000;font-size:0.84rem;font-weight:700;text-align:center;}
        .wizard-label.ativo{border-color:#8b0000;background:#fbe8e8;color:#8b0000;}
        ...
        .checkmark svg{width:100%;height:100%;stroke:#8b0000;fill:none;}
        .confirm-card h1{margin:0 0 10px;color:#8b0000;font-size:clamp(1.8rem,4vw,2.6rem);letter-spacing:-0.04em;}
        .protocol-box{margin:16px auto 12px;padding:14px;max-width:320px;border-radius:16px;background:#fbe8e8;border:2px solid #8b0000;}
        .protocol-box strong{display:block;color:#8b0000;font-size:0.98rem;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.04em;}
        .protocol-box span{display:block;color:#8b0000;font-size:1.35rem;font-weight:900;letter-spacing:0.08em;word-break:break-all;}
        .next-steps{margin:16px auto 0;max-width:460px;padding:14px;border-radius:18px;background:#fff;border:1px solid #f0c0c0;}
        .next-steps h2{margin:0 0 12px;color:#8b0000;font-size:1.2rem;}
        .next-steps ol{margin:0;padding-left:22px;color:#8b0000;line-height:1.55;list-style-position:inside;}
...
def confirmacao():
    ...
    home_url = "https://fortaleza-regional-9.onrender.com"
    return render_template_string(
